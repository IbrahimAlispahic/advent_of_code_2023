...
class CamelCard {
    ...
    static cardStrengths = ['A', 'K', 'Q', 'J', 'T', '9', '8', '7', '6', '5', '4', '3', '2', 'J']
    ...
    checkIfFiveOfAKind() {
        const jIndex = this.hand.indexOf('J')
        if (jIndex !== -1) {
            // Create a copy of the hand and remove the "J" card
            const handWithoutJ = [...this.hand];
            handWithoutJ.splice(jIndex, 1)

            return handWithoutJ.every(card => card === handWithoutJ[0])
        }
    ...
    checkIfTwoPair() {
        ...
        if (jIndex !== -1) {
            ...
            if (uniqueElements.size === 3) {
                // Check if one of the unique elements occurs exactly once
                for (const element of uniqueElements) {
                    if (handWithoutJ.filter(item => item === element).length === 1) {
                        return true
                    }
                }
            }
    ...
    checkIfOnePair() {
        ...
        if (jIndex !== -1) {
            ...
            if (uniqueElements.size === 4) {
                // Check if one of the unique elements occurs two times
                for (const element of uniqueElements) {
                    if (handWithoutJ.filter(item => item === element).length === 2) {
                        return true
                    }
